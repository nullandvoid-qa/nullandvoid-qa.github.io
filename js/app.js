(function () {
  ...
  function init() {
    ...
    const initialNavigation = () => {
      // Do NOT auto-navigate on page load - this was causing blank content / redirect issues.
      // The lesson navigation should only happen via explicit user action or route parameters.
      // Just load the stored last lesson ID into state for reference, but don't navigate.
      ...
      if (appState.currentView === 'home' && !Object.keys(appState.viewParams || {}).length > 0) {
        // Keep currentView as home - no auto-navigation
      }
    };

    // Start initial navigation immediately; this now safely loads stored state without auto-navigating
    initialNavigation();
        ...
        try {
          navigator.serviceWorker.addEventListener('message', (event) => {
            if (event.data && event.data.type === 'SW_UPDATED') {
              ...
              // Do NOT auto-reload - this was causing blank content / redirect issues
              // The page will fetch fresh content on the next network request
            }
          });
        } catch (e) {
          // noop
