      ...
      lessons: [
        {
          ...
          duration: "45 min",
          content: "<h3>O que é K6?</h3><p>K6 = ferramenta de performance testing com scripting em JavaScript. Leve, rápida, moderna.</p><h3>Por que K6?</h3><ul><li>JavaScript (fácil para QA)</li><li>Rápido e com sintaxe clara</li><li>Cloud-ready (K6 Cloud)</li><li>Open source</li></ul><h3>Instalação</h3><pre><code># macOS\nbrew install k6\n\n# Linux\napt-get install k6\n\n# Verify\nk6 version</code></pre><h3>Seu Primeiro Script</h3><pre><code>import http from 'k6/http';\nimport { check } from 'k6';\n\nexport const options = {\n  vus: 10,\n  duration: '30s',\n};\n\nexport default function() {\n  const res = http.get('https://httpbin.org/delay/1');\n  check(res, {\n    'status is 200': (r) => r.status === 200,\n    'latency &lt; 2s': (r) => r.timings.duration &lt; 2000,\n  });\n}</code></pre><h3>Rodando</h3><pre><code>k6 run script.js</code></pre><h3>Output Esperado</h3><pre><code>status is 200: 300/300\nlatency &lt; 2s: 300/300\nHTTP 200: 300/300 (100%)\n\nResultado: PASSOU</code></pre>",
          resources: [
        ...
        },
        {
          ...
          duration: "50 min",
          content: "<h3>Cenário: E-commerce Black Friday</h3><p>Você precisa testar se seu checkout aguenta o pico do Black Friday.</p><h3>Script K6 Realista</h3><pre><code>import http from 'k6/http';\nimport { check, sleep } from 'k6';\n\nexport const options = {\n  stages: [\n    { duration: '2m', target: 100 },\n    { duration: '5m', target: 100 },\n    { duration: '2m', target: 200 },\n    { duration: '5m', target: 200 },\n    { duration: '2m', target: 0 },\n  ],\n  thresholds: {\n    'http_req_duration': ['p(95)&lt;2000'],\n    'http_req_failed': ['&lt;0.01'],\n  }\n};\n\nexport default function() {\n  http.get('https://seu-ecommerce.com/');\n  sleep(1);\n\n  let res = http.get('https://seu-ecommerce.com/products/123');\n  check(res, { 'product loads': (r) => r.status === 200 });\n  sleep(2);\n\n  res = http.post('https://seu-ecommerce.com/api/cart', {\n    product_id: 123,\n    qty: 1\n  });\n  check(res, { 'add to cart: 200': (r) => r.status === 200 });\n}</code></pre><h3>Analisando Resultado</h3><p>Se P95 &gt; 2sec ou error &gt; 1%: FALHA. Precisa otimizar infraestrutura.</p>",
          resources: []
        }
      ]
    },
      ...
      lessons: [
        {
          ...
          duration: "45 min",
          content: "<h3>O que é JMeter?</h3><p>JMeter = ferramenta Apache para testes de carga distribuídos. Mais robusta que K6 para testes MUITO grandes.</p><h3>Estrutura Básica</h3><ul><li><strong>Test Plan:</strong> Container principal</li><li><strong>Thread Group:</strong> Usuários simultâneos</li><li><strong>Samplers:</strong> HTTP Request, JDBC, etc</li><li><strong>Listeners:</strong> Resultados (gráficos, tabelas)</li><li><strong>Assertions:</strong> Validações</li></ul><h3>Setup Básico</h3><p>1. Download JMeter de apache.org<br/>2. Abra jmeter.bat (Windows) ou jmeter.sh (Mac/Linux)<br/>3. Crie Test Plan</p><h3>Adicionar Thread Group</h3><pre><code>Test Plan\n└── Thread Group (Threads: 100, Ramp-up: 60s, Duration: 300s)\n    └── HTTP Request Sampler\n        └── Response Assertion</code></pre>",
          resources: [
        ...
        },
        {
          ...
          duration: "45 min",
          content: "<h3>Listeners: Vendo Resultados</h3><ul><li><strong>Summary Report:</strong> Resumo rápido (recomendado)</li><li><strong>View Results Tree:</strong> Cada requisição individualmente</li><li><strong>Graph Results:</strong> Gráfico de latência over time</li><li><strong>Response Time Graph:</strong> Visualiza P95, P99</li></ul><h3>Analisando Relatório</h3><pre><code>Label    | Samples | Avg   | Min   | Max   | Error%\n---------|---------|-------|-------|-------|-------\nlogin    | 1000    | 250ms | 100ms | 800ms | 0%\ncheckout | 1000    | 1500ms| 500ms | 3000ms| 2%\nproduct  | 1000    | 300ms | 50ms  | 2000ms| 0%\n\nProblema: checkout lento (1500ms avg, 3000ms max, 2% error)</code></pre><h3>Próximos Passos</h3><p>Checkout com problema: investigar query de banco de dados, caching, e capacidade do servidor.</p>",
          resources: []
        }
      ]
    }
